...
class AntiMentionMod(loader.Module):
    ...
    async def antimentioncmd(self, message):
        ...
        if not message.is_private:
            ...
            if not chat.admin_rights and not chat.creator:
                return await message.edit("<b>[AntiMention]</b> Я не админ здесь.")
            else:
                if chat.admin_rights.delete_messages == False:
                    return await message.edit("<b>[AntiMention]</b> У меня нет нужных прав.") 
            ...
            if chatid in am:
                ...
                return await message.edit("<b>[AntiMention]</b> Выключен в этом чате.")
            ...
            self.db.set("AntiMention", "action", ac)
            self.db.set("AntiMention", "am", am)
            return await message.edit("<b>[AntiMention]</b> Включён в этом чате.")
    ...
    async def amexcmd(self, message):
        ...
        if not message.is_private:
            ...
            if chatid not in am: return await message.edit("<b>[AntiMention]</b> Выключен в этом чате.")
            ...
            except ValueError: await message.edit("<b>[AntiMention]</b>Ошибка при поиске пользователя.")
            userid = str(user.id)
        ...
        else: return await message.edit("<b>[AntiMention]</b> Можно использовать только в чатах.")
        ...
        if not message.is_private:
            ...
            if chatid in am:
                if args:
                    ...
                    else: return await message.edit("<b>[AntiMention]</b> Такого режима нет в списке.\nДоступные режимы: kick/ban/mute/none.")
                    self.db.set("AntiMention", "action", ac)
                    return await message.edit(f"<b>[AntiMention]</b> Теперь при упоминании в чате будет выполняться действие: {ac[chatid]['action']}.")
                else: return await message.edit(f"<b>[AntiMention] </b> Настройки чата:\n\n:"
                                                f"<b>Состояние режима:</b> True\n"
                                                f"<b>При упоминании в чате будет выполняться действие:</b> {ac[chatid]['action']}\n")
            else: return await message.edit("<b>[AntiMention]</b> Выключен в этом чате.")
        else: return await message.edit("<b>[AntiMention]</b> Можно использовать только в чатах.")
